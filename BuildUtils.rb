...
class NUnitRunner
	...
	def initialize(paths)
		...
		if ENV["teamcity.dotnet.nunitlauncher"] # check if we are running in TeamCity
			...
			cp @teamCityAddinPath + '-2.4.7.dll', 'lib/nunit/addins'
		end
	
		@nunitExe = File.join('lib', "nunit-console#{(@compilePlatform.nil? ? '' : "-#{@compilePlatform}")}.exe").gsub('/','\\') + ' /nothread'
	end
...
class MSBuildRunner
    ...
    outputpath = attributes[:outputpath]
		
		frameworkDir = File.join(ENV['windir'].dup, 'Microsoft.NET', 'Framework', version)
		msbuildFile = File.join(frameworkDir, 'msbuild.exe')
		
		sh "#{msbuildFile} #{solutionFile} -p:outputpath=#{outputpath} /maxcpucount /v:m /property:BuildInParallel=false /property:Configuration=#{compileTarget} /t:Rebuild"
	end
end
...
class AsmInfoBuilder
    ...
    svn = Svn.new
    svn.retrieveRevisionNumber()
    
		@buildnumber = baseVersion + Svn.new.retrieveRevisionNumber()
...
class InstallUtilRunner
	def installServices(services, parameters)
		services.each do |service|
			...
			sh "lib/installutil /i #{params} #{service}"
		end
	end
